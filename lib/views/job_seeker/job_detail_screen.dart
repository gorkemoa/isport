  ...
  Widget _buildAppBar(JobDetailModel job) {
    return SliverAppBar(
      ...
      actions: [
        Consumer<JobViewModel>(
          builder: (context, jobVM, child) {
            final job = jobVM.currentJobDetail?.job;
            final isFavorite = job?.isFavorite ?? false;
            final isToggling = jobVM.isJobFavoriteToggling(widget.jobId);
            ...
            return Container(
              ...
              child: IconButton(
                onPressed: isToggling ? null : () {
                  HapticFeedback.lightImpact();
                  jobVM.toggleJobFavorite(widget.jobId, isFavorite);
                },
                icon: isToggling
                    ? SizedBox(
                        width: 16,
